#The following code computes the Wald based confidence interval for the Application in Lecture 2:
...
uniroot(f=LR,interval=c(0.000001,0.999999),N=25,y=0,alpha=0.05)$root # computes the upper bound at level 0.05 with observed successes y = 0 on 25 observations
...
LR.CI = function(y,n,start,alpha, ...){
   LR = function(pi.0,y,N,alpha) {
   pi.hat = y/N; L0 = 0; L1 = 0 
   if (pi.0 < 1) L0 = L0 + (N-y)*log(1-pi.0)
   if (pi.0 > 0) L0 = L0 + y*log(pi.0)
...
LR.CI(0,25,.2,0.05)$estimate # computes the upper bound at level 0.05 with observed successes y = 0 on 25 observations
...
LR.CI(5,25,.1,0.05)$estimate # lower bound
LR.CI(5,25,.4,0.05)$estimate # upper bound
...
#Score statistic:
      ...
      zscore = (y-n*.25)/sqrt(n*.25*.75)
      zscore
      abs(zscore) > qnorm(0.975)

#score p-value and 95% CI

      prop.test(y,n,p=0.25,correct=F)
